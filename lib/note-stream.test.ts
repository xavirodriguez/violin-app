...
const createMockRawPitchStream = (events: Array<RawPitchEvent>) =>
  async function* () {
...
describe('createPracticeEventPipeline', () => {
  const mockTargetNote: TargetNote = {
    pitch: { step: 'A', octave: 4, alter: null },
    duration: 'quarter',
  }
  ...
  it('should emit NOTE_MATCHED after a stable correct note', async () => {
    const pitchEvents: RawPitchEvent[] = [
  ...
  it('should not emit NOTE_MATCHED if note is unstable', async () => {
    const pitchEvents: RawPitchEvent[] = [
      ...
      { pitchHz: 0, confidence: 0, rms: 0, timestamp: 1100 }, // Silence resets stability
