...
describe('TechniqueAnalysisAgent', () => {
  ...
  const createFrames = (durationMs: number, centsFunc: (t: number) => number, rmsFunc: (t: number) => number): TechniqueFrame[] => {
  ...
  it('should calculate base stability correctly', () => {
    const frames = createFrames(500, () => 10, () => 0.05)
  ...
  it('should detect vibrato rate and width', () => {
    const rate = 6
    const width = 20
    const frames = createFrames(1000, (t) => (width/2) * Math.sin(2 * Math.PI * rate * (t/1000)), () => 0.1)
  ...
  it('should gate vibrato if pitch stability is low', () => {
    ...
    const frames = createFrames(1000, (t) => (t % 100 < 50 ? 50 : -50), () => 0.1)
    const segment: NoteSegment = {
      noteIndex: 0,
      targetPitch: 'A4',
      startTime: 0,
  ...
  it('should detect resonance/wolf-tone evidence', () => {
    const frames = createFrames(500, () => 0, () => 0.05).map(f => ({ ...f, confidence: 0.4 }))
    const segment: NoteSegment = {
      noteIndex: 0,
      targetPitch: 'A4',
      startTime: 0,
  ...
  it('should calculate rhythm metrics', () => {
    const frames = createFrames(500, () => 0, () => 0.1)
    const segment: NoteSegment = {
      noteIndex: 1,
      targetPitch: 'A4',
      startTime: 1050,
