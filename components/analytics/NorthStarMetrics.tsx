  ...
  return (
    <div className="mt-12 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-2xl font-bold">Engagement North Star</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricSummary
          ...
          description="Sessions with real audio feedback"
          color="text-indigo-600"
        />
        <MetricSummary
          icon={TrendingUp}
          label="Consistency Rate"
          value={`${progress.currentStreak > 0 ? 'Active' : 'Idle'}`}
          description={`${progress.currentStreak} day current streak`}
          color="text-emerald-600"
        />
        <MetricSummary
          icon={BarChart3}
          label="Onboarding"
          value={hasOnboarding ? "Complete" : "Pending"}
      ...
      <Card className="p-4 bg-indigo-50/50 border-indigo-100">
        <p className="text-xs text-indigo-700 leading-relaxed">
          <b>Pedagogical Insight:</b> Our "North Star" measures meaningful practice. A valid session requires at least one note matched with real-time pitch detection.
        </p>
      </Card>
    </div>
  )
