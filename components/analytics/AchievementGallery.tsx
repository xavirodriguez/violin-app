...
export function AchievementGallery() {
  ...
  const t = useTranslation(language).achievements
    ...
    <div className="mt-12 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm font-medium text-muted-foreground">
          {unlockedIds.size} / {ACHIEVEMENT_DEFINITIONS.length} {t.unlocked}
        </div>
      </div>
          ...
          return (
            <div key={cat} className="space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground border-b pb-2 flex items-center justify-between">
                <span>{t[cat as keyof typeof t]} {t.gallery.split(' ')[0]}</span>
