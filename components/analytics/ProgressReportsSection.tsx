...
export function ProgressReportsSection() {
  ...
  const t = useTranslation(language).reports
  ...
  return (
    <Card className="p-8 bg-white border-2 print:border-none print:shadow-none shadow-sm space-y-8">
      ...
      <div className="flex justify-between items-start border-b pb-6">
        <div>
          <h3 className="text-3xl font-black text-slate-900">{t.mainTitle}</h3>
        ...
        <div className="text-right">
          ...
          <div className="text-slate-500 text-xs">{t.generated}: {new Date(report.generationDate).toLocaleDateString()}</div>
        </div>
      </div>

      {/* Summary Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricBox label={t.practiceTime} value={`${report.summary.totalPracticeMinutes}m`} sub="Total cumulative" />
        <MetricBox label={t.sessions} value={report.summary.sessionsCount} sub="Completed" />
        <MetricBox label={t.avgAccuracy} value={`${report.summary.averageAccuracy}%`} sub="Last 30 days" />
        <MetricBox label={t.streak} value={`${report.summary.activeStreak}d`} sub="Consistency" />
      </div>
