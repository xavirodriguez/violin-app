...
export function TunerDisplay({ note, cents, confidence }: TunerDisplayProps) {
  ...
  /** Helper to generate descriptive status text for screen readers. */
  const getStatusText = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <TunerStatusText note={note} cents={cents} isInTune={isInTune} isClose={isClose} />
    </div>
  )
}
...
function TunerStatusText({
  ...
  if (!note || cents === null) return null
