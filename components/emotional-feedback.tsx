...
/**
 * Props for the EmotionalFeedback component.
 */
interface EmotionalFeedbackProps {
  centsOff: number | null
  isInTune: boolean
  noteMatches: boolean
  status: 'listening' | 'validating' | 'correct' | 'idle' | string
}
...
function EmojiBasedFeedback({
  ...
  const EMOJI_MAP: Record<EmotionalState, { emoji: string; message: string; color: string }> = {
    perfect: { emoji: '🎉', message: '¡Perfecto!', color: 'text-green-500' },
    great: { emoji: '😊', message: '¡Muy bien!', color: 'text-green-400' },
    close: { emoji: '😐', message: 'Casi...', color: 'text-yellow-500' },
    offTrack: { emoji: '😕', message: 'Ajusta un poco', color: 'text-orange-500' },
    wrongNote: { emoji: '🤔', message: 'Nota incorrecta', color: 'text-red-500' },
    listening: { emoji: '👂', message: 'Tocando...', color: 'text-muted-foreground' },
  }
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-muted-foreground mt-2 text-sm">
        {isInTune ? 'Afinado' : centsOff > 0 ? 'Demasiado agudo' : 'Demasiado grave'}
      </p>
    </div>
  )
}
